...
export function MembershipsClient({
  ...
  const [activeTab, setActiveTab] = useState<"memberships" | "plans">(
    "memberships",
  );
  ...
  function addPlanService() {
    const svcId = parseInt(addServiceId, 10);
    if (!svcId) {
      toast.error("Select a service");
      return;
    }
    if (planServices.find((ps) => ps.serviceId === svcId)) {
    ...
    setPlanServices((prev) => [
      ...prev,
      {
        ...
        allowedCount: parseInt(addAllowedCount, 10) || 1,
      },
    ]);
    setAddServiceId("");
    setAddAllowedCount("1");
  }
  ...
  async function handleSavePlan() {
    ...
    try {
      if (editPlan) {
        ...
        setPlans((prev) =>
          prev.map((p) =>
            p.id === updated.id ? { ...updated, services: planServices } : p,
          ),
        );
                          ...
                          <div key={svc.serviceId}>
                            ...
                            <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <motion.div
                                initial={{ width: 0 }}
                                animate={{
                                  width: `${Math.min((svc.used / svc.allowed) * 100, 100)}%`,
                                }}
                                transition={{
                                  duration: 0.6,
            ...
            >
              {plans.length === 0 ? (
                <EmptyState
                  ...
                  description="Create membership plans to offer customers"
                  action={
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setAssignForm({ ...assignForm, planId: e.target.value })
              }
              className="w-full h-9 rounded-lg border border-slate-200 bg-slate-50 px-3 text-[13px] text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setPlanForm({ ...planForm, validityDays: e.target.value })
                }
                min="1"
                className="w-full h-9 rounded-lg border border-slate-200 bg-slate-50 px-3 text-[13px] focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
          ...
          <div>
            ...
            <div className="flex gap-2 mb-3">
              ...
              >
                ...
                {services
                  .filter(
                    (s) => !planServices.find((ps) => ps.serviceId === s.id),
                  )
                  .map((s) => (
              ...
              <input
                type="number"
                value={addAllowedCount}
                onChange={(e) => setAddAllowedCount(e.target.value)}
                min="1"
                placeholder="Times"
              ...
              />
              <Button variant="outline" size="sm" onClick={addPlanService}>
                <Plus size={13} />
              </Button>
            </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setPlanServices((prev) =>
                            prev.filter((p) => p.serviceId !== ps.serviceId),
                          )
                        }
