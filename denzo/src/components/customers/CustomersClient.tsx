...
import { Search, Plus, History, PenLine, Users, X } from "lucide-react";
...
import type { Customer, Bill, BillItemData } from "@/types";

interface CustomerHistoryData {
  customer: Customer;
  totalVisits: number;
  bills: Array<{ id: number; date: string; totalAmount: number; items: BillItemData[] }>;
}
...
export function CustomersClient({ initialCustomers }: { initialCustomers: Customer[] }) {
  ...
  function openAdd() { setForm({ name: "", phone: "", email: "" }); setEditCustomer(null); setShowForm(true); }
  function openEdit(c: Customer) { setForm({ name: c.name, phone: c.phone, email: c.email ?? "" }); setEditCustomer(c); setShowForm(true); }
  function closeForm() { setShowForm(false); setEditCustomer(null); }
  ...
  return (
    <PageTransition>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="primary" size="md" onClick={openAdd}><Plus size={15} />Add Customer</Button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          {filtered.length === 0 ? (
            <EmptyState icon={<Users size={22} />} title={searchQuery ? "No results found" : "No customers yet"} description={searchQuery ? `No customers match "${searchQuery}"` : "Add your first customer to get started"} action={!searchQuery ? <Button variant="primary" size="sm" onClick={openAdd}><Plus size={13} />Add Customer</Button> : undefined} />
          ) : (
                ...
                <tbody>
                  {filtered.map((c, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-[13px] text-slate-500">{c.email ?? <span className="text-slate-300">—</span>}</td>
      ...
      {/* Add/Edit Modal */}
      <Modal isOpen={showForm} onClose={closeForm} title={editCustomer ? "Edit Customer" : "Add Customer"} size="sm">
        <div className="space-y-4">
          ...
          {!editCustomer && (
            <div>
              ...
              <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="10-digit phone number" maxLength={10}
                className="w-full h-9 rounded-lg border border-slate-200 bg-slate-50 px-3 text-[13px] text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
          )}
          <div>
            <label className="text-[12px] font-medium text-slate-600 mb-1.5 block">Email <span className="text-slate-400">(optional)</span></label>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <Button variant="primary" size="md" className="flex-1" onClick={handleSave} loading={saving}>{editCustomer ? "Save Changes" : "Add Customer"}</Button>
          </div>
        </div>
      </Modal>

      {/* History Modal */}
      <Modal isOpen={!!historyCustomer} onClose={() => setHistoryCustomer(null)} title={historyCustomer ? `${historyCustomer.name}'s Visit History` : ""} size="lg">
        {historyLoading ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-indigo-50 rounded-xl">
              ...
              <div>
                <p className="text-[13px] font-semibold text-slate-800">{historyData.totalVisits} total visits</p>
                <p className="text-[11px] text-slate-500">{historyCustomer?.phone}</p>
              </div>
            </div>
            {historyData.bills.length === 0 ? (
              <p className="text-center text-[13px] text-slate-400 py-6">No bills yet</p>
            ) : (
              <div className="space-y-3">
                {historyData.bills.map((bill) => (
                  <div key={bill.id} className="rounded-xl border border-slate-100 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-[12px] text-slate-500">{formatDate(bill.date)}</p>
                      <p className="text-[14px] font-bold text-slate-800">₹{bill.totalAmount}</p>
                    </div>
                    <div className="space-y-1.5">
                      {bill.items.map((item) => (
                        <div key={item.id} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-[12px] text-slate-700">{item.serviceName}</span>
                            <span className="text-[11px] text-slate-400">by {item.employeeName}</span>
                          </div>
                          {item.isMembershipService
                            ? <Badge variant="free">FREE</Badge>
                            : <span className="text-[12px] font-medium text-slate-700">₹{item.price}</span>
                          }
                        </div>
                      ))}
                    </div>
