...
export function MarketingClient({ customerCount, initialCampaigns }: MarketingClientProps) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setCampaigns((prev) => [
        { id: json.campaignId ?? Date.now(), message: message.trim(), sentAt: new Date().toISOString(), recipientCount: json.sent },
        ...prev,
      ]);
      setMessage("");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[55fr_45fr] gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
              ...
              <h2 className="text-[15px] font-semibold text-slate-800">Compose Message</h2>
            </div>
            <div className="p-6 space-y-5">
              <div>
                <p className="text-[12px] font-semibold text-slate-500 uppercase tracking-wide mb-3">Quick Templates</p>
                  ...
                  {SMS_TEMPLATES.map((tpl, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[12px] font-semibold ${selectedTemplateIdx === idx ? "text-indigo-700" : "text-slate-700"}`}>{tpl.label}</span>
                      </div>
                      <p className="text-[11px] text-slate-400 leading-relaxed">{tpl.text.slice(0, 55)}...</p>
                    </button>
                  ))}
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-[11px] font-medium ${message.length > 160 ? "text-rose-500" : "text-slate-400"}`}>{message.length} chars</span>
                </div>
                <textarea
                  rows={5}
                  value={message}
                  onChange={(e) => { setMessage(e.target.value); setSelectedTemplateIdx(null); }}
              ...
              {message.trim() && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-6 h-6 rounded-full bg-indigo-200 flex items-center justify-center"><MessageSquare size={11} className="text-indigo-700" /></div>
                    <p className="text-[11px] font-semibold text-indigo-600 uppercase tracking-wide">Preview</p>
                  </div>
                  <p className="text-[13px] text-slate-700 leading-relaxed whitespace-pre-wrap">{message}</p>
                </motion.div>
              )}
              <Button variant="primary" size="lg" className="w-full" onClick={() => { if (!message.trim()) { toast.error("Write a message first"); return; } setShowConfirm(true); }} disabled={!message.trim()}>
                <Send size={15} />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-[15px] font-semibold text-slate-800">Campaign History</h2>
              </div>
              {campaigns.length > 0 && (
                <span className="text-[11px] font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{campaigns.length} sent</span>
              )}
            </div>
            <div className="p-5">
              {campaigns.length === 0 ? (
                <EmptyState icon={<MessageSquare size={22} />} title="No campaigns yet" description="Send your first SMS campaign to reach all your customers." />
              ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
                          <Clock size={11} />
                          <span>{new Date(c.sentAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
                        </div>
                        <div className="flex items-center gap-1 text-[11px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full flex-shrink-0">
                          <Users size={10} />{c.recipientCount}
                        </div>
                      </div>
                      <p className="text-[12px] text-slate-600 leading-relaxed">{c.message.length > 80 ? `${c.message.slice(0, 80)}...` : c.message}</p>
                    </motion.div>
                  ))}
                </div>
              )}
