    ...
    <PageTransition>
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          {/* Filter pills */}
          <div className="px-4 sm:px-5 py-3 flex items-center gap-2 border-b border-slate-50 overflow-x-auto scrollbar-hide">
            {MODES.map((mode) => (
              <button
                key={mode}
                type="button"
              ...
              >
                {mode === "Cash" ? "💵 Cash" : mode === "Card" ? "💳 Card" : mode === "Online" ? "📱 Online" : mode === "Membership" ? "🎫 Membership" : "🧾 All"}
              </button>
            ))}
          </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="text-[13px] font-semibold text-slate-800 truncate">{bill.customerName}</p>
                          <p className="text-[11px] text-slate-400">{bill.customerPhone} · #{bill.id}</p>
                          <p className="text-[11px] text-slate-400 mt-0.5">{formatDate(bill.date)}</p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className="text-[15px] font-bold text-slate-800">₹{bill.totalAmount.toLocaleString("en-IN")}</p>
                          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-lg border mt-1 inline-block ${cfg?.color ?? "bg-slate-100 text-slate-600 border-slate-200"}`}>
                      ...
                      <div className="mt-2 flex flex-wrap gap-1">
                        {bill.items.map((item) => (
                          <span key={item.id} className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md">
                            {item.serviceName}
                            {item.isMembershipService && <span className="ml-1 text-indigo-500 font-bold">M</span>}
                          </span>
                        ))}
                      </div>
                    </motion.div>
