...
import { Plus, Scissors, PenLine, Trash2 } from "lucide-react";
...
export function ServicesClient({ initialServices }: { initialServices: Service[] }) {
  ...
  function openAdd() { setForm({ name: "", basePrice: "" }); setEditService(null); setShowForm(true); }
  function openEdit(s: Service) { setForm({ name: s.name, basePrice: String(s.basePrice) }); setEditService(s); setShowForm(true); }
  function closeForm() { setShowForm(false); setEditService(null); }

  async function handleSave() {
    if (!form.name.trim() || !form.basePrice || Number(form.basePrice) <= 0) {
      toast.error("Please enter a valid service name and price"); return;
    }
    setSaving(true);
    try {
      if (editService) {
        const res = await fetch(`/api/services/${editService.id}`, {
          ...
          body: JSON.stringify({ name: form.name, basePrice: Number(form.basePrice) }),
        });
    ...
    <PageTransition>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[13px] text-slate-500">{services.length} service{services.length !== 1 ? "s" : ""} available</p>
          </div>
          <Button variant="primary" size="md" onClick={openAdd}><Plus size={15} />Add Service</Button>
        </div>
        ...
        {services.length === 0 ? (
          <EmptyState
            ...
            action={<Button variant="primary" size="sm" onClick={openAdd}><Plus size={13} />Add Service</Button>}
          />
        ) : (
          <motion.div
          ...
          >
            {services.map((s) => (
              ...
              >
                ...
                {/* Content */}
                <p className="text-[14px] font-semibold text-slate-800 mb-1 truncate pr-4">{s.name}</p>
                <p className="text-[24px] font-bold text-indigo-600">{formatCurrency(s.basePrice)}</p>
                <p className="text-[11px] text-slate-400 mt-0.5">base price per session</p>
                ...
                <div className="absolute bottom-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                  <button type="button" onClick={() => openEdit(s)} className="p-1.5 rounded-lg bg-indigo-50 text-indigo-500 hover:bg-indigo-100 transition-colors">
                    <PenLine size={13} />
                  </button>
                  <button type="button" onClick={() => setDeleteId(s.id)} className="p-1.5 rounded-lg bg-rose-50 text-rose-500 hover:bg-rose-100 transition-colors">
                    <Trash2 size={13} />
                  </button>
                </div>
              </motion.div>
            ...
            >
              ...
              <span className="text-[13px] font-medium text-slate-500">Add Service</span>
            </motion.button>
          </motion.div>
        )}
      </div>
      ...
      {/* Form Modal */}
      <Modal isOpen={showForm} onClose={closeForm} title={editService ? "Edit Service" : "Add Service"} size="sm">
        <div className="space-y-4">
          <div>
            <label className="text-[12px] font-medium text-slate-600 mb-1.5 block">Service Name *</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="text-[12px] font-medium text-slate-600 mb-1.5 block">Base Price (₹) *</label>
